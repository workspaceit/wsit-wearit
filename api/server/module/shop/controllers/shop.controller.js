  ...
  try {
    ...
    if (isAdmin) {
      query.populate('verificationIssue')
        ...
        .populate('storeDetailsId')
        .populate('productsImg')
    }
...
exports.search = async (req, res, next) => {
  ...
  try {
    ...
    if (query.featured && ["false", "0"].indexOf(query.featured) === -1) {
      query.featured = true;
    }

    if (req.query.isWholeSaler == "true" || req.query.isWholeSaler == true) {
      query.wholeSeller = true;
    } else if (req.query.isWholeSaler == "false") {
      query.$or =
        [
          { wholeSeller: false },
          { wholeSeller: { $exists: false } }
        ]
    } else {
      query.$or =
        [
